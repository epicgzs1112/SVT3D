...
import numpy as np
from svt.svt import Block_forde as SVT_Block
...
class TransformerDecoder(torch.nn.Module):
    def __init__(
        ...
        # self.svtblock1 = SVT_Block(embed_dim,
        #                           num_heads,
        #                           mlp_ratio,
        #                           drop_path=0.,
...
#
       ...
       #self.svtblock3 = SVT_Block(embed_dim // 8,
       #                           num_heads,
       #                           mlp_ratio,
       #                           drop_path=0.,
       #                           norm_layer=nn.LayerNorm,
     ...
     #   self.svtblock4 = SVT_Block(embed_dim // 4,
     #                              num_heads,
     #                              mlp_ratio,
     #                              drop_path=0.,
     #                              norm_layer=nn.LayerNorm,
...
#
    ...
    def forward(self, context):
       ...
       # context=self.svtblock1(context,8,8)
        ...
        for blk in self.blocks2: # LR
            context = blk(context)
       # print(f'\n context:{context.shape}')
        #context = self.svtblock2(context,256,256)
      ...
      #  print(f'\n context:{context.shape}')
     #   context_temp = self.svtblock3(context_temp)
        ...
        for blk in self.blocks4: # HR
            context_temp = blk(context_temp)
       # print(f'\n context:{context.shape}')
      #  context_temp = self.svtblock4(context_temp)
        context_temp = rearrange(
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        self.svtblock = torch.nn.Sequential(SVT_Block(embed_dim,
                                                   num_heads,
                                                   mlp_ratio,
                                                  drop_path=0.,
                                                norm_layer=nn.LayerNorm,
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, context):
        # [B, P, D]
        #print(f'\n before context:{context.shape}')
        context = self.prepare(context=context)
        #print(f'\n after preare :{context.shape}')
        context = self.layer_norm(context)
       # #print(f'\n decioder context:{context.shape}')
        out = self.transformer_decoder(context=context)
        #print(f'\n decoder out :{out.shape}')
        return out
