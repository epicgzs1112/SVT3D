...
class VisionTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.svtblock = torch.nn.Sequential(SVT_Block(embed_dim,
                                  ...
                                  sr_ratio=1,
                                  block_type='scatter'))
        self.svtblock2 = torch.nn.Sequential(SVT_Block(embed_dim,
                                                      num_heads,
                                                      mlp_ratio,
                                                      drop_path=0.,
                                                      norm_layer=nn.LayerNorm,
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def prepare(self, x):
        batch_size, view_num = x.shape[0:2]
        #print(f'\n x before com   batch and view :{x.shape}')
        x = rearrange(x, 'b v c h w -> (b v) c h w')  # [B*V, C, H, W]
        #print(f'\n x after com   batch and view :{x.shape}')
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            if layer in self.lga_layer:
             ...
             #   print(f'\nencoder x after lga block{x.shape}')
            else:
