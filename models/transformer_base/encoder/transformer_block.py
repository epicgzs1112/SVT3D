...
# Refer LRGT <https://github.com/LiyingCV/Long-Range-Grouping-Transformer>
...
from svt.svt import Block as SVT_Block
...
class STMBlock(torch.nn.Module):
    def __init__(self,
       ...
       # self.svtblock=torch.nn.Sequential(SVT_Block(  dim,
        #num_heads,
       # mlp_ratio,
       # drop_path=0.,
    ...
    def cluster(self, feature, batch_size):
        with torch.no_grad():
          ...
          #  distances = torch.cdist(feature, feature)/(self.dim ** 0.5)
            sigma = torch.std(feature, dim=1, keepdim=True)/(self.dim ** 0.5)

            sigma_sum=torch.sum(sigma)
            #print(f'\n distance :{distances.shape},sigma:{sigma.shape},sigma_sum:{sigma_sum.shape}')
            sigma=sigma/sigma_sum
            feature=feature*sigma
            distance_matrix = torch.cdist(feature, feature) / (self.dim ** 0.5)
            #distance_matrix = distances / sigma
            # get local density
    ...
    def merger(self, feature, idx_cluster, token_score, batch_size, patch_num):
        # feature 4 588 768 idx 4 588  token_sc 4 588 1
       ...
       # print(f'\n idx_batch:{idx_batch.shape}')
        ...
        #print(f'\n norm_weights :{norm_weight.shape}')
        # average token features
        merged_feature = feature.new_zeros(batch_size * self.out_token_len, self.dim)
        #print(f'\n merger fea :{merged_feature.shape}')
        source = feature * norm_weight
        #print(f'\n source :{source.shape}')
        #print(f'\nsource.reshape(batch_size * patch_num, self.dim){source.reshape(batch_size * patch_num, self.dim).shape}')
        ...
        #print(f'\n merged_feature:{merged_feature.shape}')
    ...
    def transformer_block(self, q_input, kv_input, token_score):
        ...
        #attn2=self.svtblock(q_input)
        #feature2=q_input+attn2
    ...
    def forward(self, x):
        # feature [B, V*P, D]
        #print(f'\n x in merger_block:{x.shape}')#4 3*196 768 = 4 588 768
        batch_size, patch_num, _ = x.shape

        x = self.norm1(x)
        #print(f'\n x in stm_blk after norm:{x.shape}')
        token_score = self.score_mlp(x)  
        #print(f'token_score after scoremlp:{token_score.shape}')#4 588 1
        idx_cluster = self.cluster(x, batch_size)
        #print(f'\n id_cluster:{idx_cluster.shape}')#4 588
        q_input = self.merger(x, idx_cluster, token_score, batch_size, patch_num)
        #print(f'\n q_input:{q_input.shape}')#4 196 768
        kv_input = x
        #print(f'\n kv_input:{kv_input.shape}')#4 588 768
        feature = self.transformer_block(q_input, kv_input, token_score)  # [B, OUT_TOKEN_LEN, D]
        #print(f'\n  merger_block return :{feature.shape}')
        return feature
