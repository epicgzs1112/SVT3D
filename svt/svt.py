...
class SVT_channel_mixing_fordecoder(nn.Module):
    def __init__(self, dim):
        ...
        if dim == 96:  # 96 for large model, 64 for small and base model
            ...
            self.block_size = self.hidden_size // self.num_blocks
            assert self.hidden_size % self.num_blocks == 0
            self.complex_weight_ll = nn.Parameter(torch.randn(dim, 256, 256, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_1 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_2 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
        ...
        if dim == 768:
            ...
            self.block_size = self.hidden_size // self.num_blocks
            assert self.hidden_size % self.num_blocks == 0
            self.complex_weight_ll = nn.Parameter(torch.randn(dim, 14, 14, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_1 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_2 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
...
class SVT_channel_token_mixing(nn.Module):
    def __init__(self, dim):
        ...
        if dim == 768:
            ...
            self.complex_weight_ll = nn.Parameter(torch.randn(dim // 2, 14, 14, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_1 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
            self.complex_weight_lh_2 = nn.Parameter(
                torch.randn(2, self.num_blocks, self.block_size, self.block_size, dtype=torch.float32) * 0.02)
...
class Block_forde(nn.Module):
    def \
        ...
        if block_type == 'std_att':
            ...
            self.attn = SVT_channel_mixing_fordecoder (dim)
            #self.attn = SVT_token_mixing (dim)
            #self.attn = SVT_channel_token_mixing (dim)
        self.mlp = PVT2FFN(in_features=dim, hidden_features=int(dim * mlp_ratio))
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class SVT(nn.Module):
    ...
    def forward_cls(self, x):
        ...
        #print(f'\n x in forwardcls:{x.shape}')
        for block in self.post_network:
            x = block(x)
         #   print(f'\n x in forwardcls_block:{x.shape}')
        return x

    def forward_features(self, x):
        #print(f'\n x begin in svt :{x.shape}')
        B = x.shape[0]
        for i in range(self.num_stages):
            patch_embed = getattr(self, f"patch_embed{i + 1}")
            block = getattr(self, f"block{i + 1}")
            x, H, W = patch_embed(x)
            #print(f'\n x after patch_embed:x={x.shape}')
            for blk in block:
                x = blk(x, H, W)
               # print(f'\n x after block:x={x.shape}')
        ...
        #print(f'\n x after block :{x.shape}')
        #x = self.forward_cls(x)[:, 0]
        #print(f'\n x after forward_cls :{x.shape}')
